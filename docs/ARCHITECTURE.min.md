...
## What it is
A single-tenant Bun daemon that orchestrates AI coding-agent CLIs (`claude`, `codex`, `gemini`) through a planner → wave-execution → iteration loop. Each task runs in its own git worktree. State lives under `.orquesta/crew/`. Three optional front-ends (Web UI, standalone Web UI server, Go TUI) sit on top of the same REST + WebSocket API.
...
src/
  ...
  cli/         orq.ts — plan / approve / start / status / logs / doctor
  ...
  daemon/      orchestrator, task-pipeline, task-closure, iteration-manager,
               ask-router, planner-service, index.ts (boot)
...
- `Plan` — a run. Status: `drafting → awaiting_approval → approved → running → done|failed`.
...
- `Agent` — one spawned CLI process bound 1:1 to a subtask (or planner). Status `idle | working | live | dead`.
...
1. **Planning** — planner agent emits a DAG via the `emit_tasks` MCP tool → `awaiting_approval`.
...
Boots `PlanStore`, session token, `Journal` (SQLite), `Bus`, `AgentPool`, `AskRouter`, `TaskPipeline`, `IterationManager`, `Orchestrator`, `PlannerService`, MCP handler, HTTP handler, WS handlers; calls `recoverInterruptedRun()` on every boot. The daemon does **not** bundle the UI — it serves prebuilt assets from `dist/ui/`.
...
- `emit_tasks` (planner | architect | pm | qa) → validates DAG (`detectCycle`), persists, emits `tasks_emitted`.
- `broadcast` (pm) → writes to a target agent's PTY stdin.

## HTTP / WS (`src/api/`)
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/health`, `/api/diagnostics`, `/api/planner/diagnostics`, `/api/export` | Liveness / state dumps |
...
| POST | `/api/plan`, `/api/plan/reset`, `/api/approve` | Run lifecycle |
...
## Env knobs
`ORQ_PORT` (8000), `ORQ_HOST` (127.0.0.1), `ORQ_CORS_ORIGIN`, `ORQ_UI_PORT` (4173), `VITE_DAEMON_URL`, `ORQ_DAEMON_URL`, `ORQ_AUTONOMOUS`, `ORQ_SUBTASK_TIMEOUT_MS`, `ORQ_ROLE_TIMEOUT_MS`, `ORQ_PLANNER_TIMEOUT_MS`, `ORQ_INITIAL_PROMPT_DELAY_MS`, `ORQ_INITIAL_PROMPT_SUBMIT_DELAY_MS`, `ORQ_CWD` (used by `scripts/dev.sh`).
